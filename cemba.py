...
def atac_samples_to_loom(base_dir,
    ...
    for sample in samples:
        ...
        if len(replicates) > 0:
            ...
            if verbose:
                logger.warning('No replicates found in {}'.format(sample))
            replicates = [tmp_base]
        for replicate in replicates:
            ...
            if verbose:
                logger.info('Processing {}'.format(count_fn))
            # Read in data
            ...
            cells = np.loadtxt(fname = col_fn,
                               dtype = str,
                               delimiter = '\t')
            counts = sparse.load_npz(file = count_fn)
            ...
            if bins:
                ...
                counts = general_utils.expand_sparse(mtx = counts,
                                                     col_index = None,
                                                     row_index = new_idx,
                                                     col_N = None,
            ...
            if first_iter:
                loompy.create(filename = loom_file,
                              layers = {'': sparse.coo_matrix(counts.shape,dtype=int),
                                        'counts':counts},
                              row_attrs = row_attrs,
    ...
    if verbose:
        ...
        logger.info('Generated {0} in {1:.2f} {2}'.format(loom_file,time_run,time_fmt))
    ...
    for sample in samples:
        ...
        for replicate in replicates:
            ...
            tmp_qc = pd.read_table(rep_file, 
                                   ...
                                   header = 0,
                                   index_col = 0)
            tmp_qc.index = gen_atac_barcode(barcodes = tmp_qc.index.values.astype(str),
                                            sample = curr_sample)
            qc_df.append(tmp_qc)
...
def add_atac_qc(loom_file, 
    ...
    Args:
        ...
        uniq_num (int): Mininum number of unique reads for a cell to be considered
        uniq_rate (float): Mininum percentage of unique reads to total reads (0-1)
    ...
    """
    if verbose:
        logger.info('Getting quality control information')
        t0 = time.time()
    ...
    if verbose:
        t1 = time.time()
        time_run, time_fmt = general_utils.format_run_time(t0,t1)
        logger.info('Added QC {0} in {1:.2f} {2}'.format(loom_file,time_run,time_fmt))
...
def merge_bins_cell(filename,
    ...
    Args:
        ...
        compression (str): File compression (follows pandas read_table convention)
    ...
    if merge_bins % curr_bins != 0:
        raise ValueError('{0} is not divisible by {1}'.format(merge_bins,curr_bins))
    # Read file
    df = pd.read_table(filename,
                       ...
                       dtype=bin_dtype)
    # Get chromosomes
    chrom_lengths = general_utils.get_mouse_chroms(prefix = chr_prefix, include_y=False)
    chrs_all = np.asarray([])
    bins_all = np.asarray([])
...
def sample_to_loom(sample_file,
    ...
    Args:
        ...
        merge_bins (int): Proposed bin size for merged bins
            If curr_bins and merge_bins are provided, will merge bins to this size
        double_x (bool): If merging bins, will double size of bins on X chromosome
        chr_prefix (bool): If true, expects chromosomes in the format of chrX not X
        sep (str): File separator (follows pandas read_table convention)
        compression (str): File compression (follows pandas read_table convention)
    ...
    if bins:
        if curr_bins != merge_bins and merge_bins is not None:
            ...
            count_df = pd.read_table(sample_file,
                                 ...
                                 index_col = None,
                                 dtype = gene_dtype)
        count_df['Accession'] = count_df['chr'] + '_' + count_df['bin'].astype(str)
    ...
    layers = {'':np.zeros((count_df.shape[0],1),dtype=int),
              'mC_CHH':np.expand_dims(count_df['mCH'].values,1),
    ...
    if append:
        ...
        if os.path.exists(loom_file):
            logger.warning('{} already exists, overwriting'.format(loom_file))
        loompy.create(filename = loom_file,
                      layers = layers,
                      row_attrs = row_attrs,
                      col_attrs = col_attrs)
...
def cemba_samples_to_loom(base_dir,
    ...
    if isinstance(samples,str):
        samples = [samples]
    if os.path.exists(loom_file):
        raise OSError('{} already exists!'.format(loom_file))
    if verbose:
        logger.info('Writing {}'.format(loom_file))
        t0 = time.time()
    # Loop over samples
    ...
    for sample in samples:
        ...
        for filename in fn_list:
            cell_id = np.asarray([re.sub('\.tsv\.bgz','',os.path.basename(filename))])
            if bins: #Easy to break
                cell_id = np.core.defchararray.replace(a = cell_id,
                                                       old = 'binc_',
                                                       new = '')
                cell_id = np.core.defchararray.replace(a = cell_id,
                                                       old = '_{}'.format(curr_bins),
                                                       new = '')
            else:
                cell_id = np.core.defchararray.replace(a = cell_id,
                                                       old = 'genebody_',
    ...
    if verbose:
        t1 = time.time()
        time_run, time_fmt = general_utils.format_run_time(t0,t1)
        logger.info('Wrote {0} in {1:.2f} {2}'.format(loom_file,time_run,time_fmt))
...
def ensemble_to_loom(ensemble_file,
    ...
    if bins:
        ...
        ra = 'Accession'
    if verbose:
        logger.info('Loading {0} file: {1}'.format(msg, ensemble_file))
    ...
    if verbose:
        ...
        logger.info('Read file in {0:.2f} {1}'.format(time_run,time_fmt))
    ...
    # Get mC and C
    if verbose:
        logger.info('Splitting mC and C')
    ...
    if verbose:
        ...
        logger.info('Split in {0:.2f} {1}'.format(time_run,time_fmt))
    # Error check
    if ((not np.all(np.equal(df_c.columns,df_mc.columns))) or 
        (not np.all(np.equal(df_c.index,df_mc.index)))):
        raise ValueError('df_c and df_mc should have matching labels')
    # Generate loom file
    ...
    if verbose:
        ...
        logger.info('Wrote file in {0:.2f} {1}'.format(time_run,time_fmt))
...
def read_methylation_qc(base_dir,
    ...
    qc_df = []
    for sample in samples:
        base_loop = '{0}/{1}'.format(base_dir,sample)
        tmp_qc = pd.read_table('{0}/{1}/mapping_summary_{1}.tsv'.format(base_dir,sample),
                               sep = '\t',
                               header = 0,
                               index_col = 0)
        qc_df.append(tmp_qc)
...
def add_methylation_qc(loom_file, 
    ...
    """
    if verbose:
        logger.info('Adding QC to {}'.format(loom_file))
        t0 = time.time()
    ...
    with loompy.connect(loom_file) as ds:        
        ...
        ds.ca['Genome_Coverage'] = res_qc['% Genome covered'].values.astype(float)
    if verbose:
        t1 = time.time()
        time_run, time_fmt = general_utils.format_run_time(t0,t1)
        logger.info('Added QC in {0:.2f} {1}'.format(time_run,time_fmt))
...
def allen_category_df(label_list,
    ...
    Returns:
        category_df (dataframe): Contains look_up between Allen's labels and a user's
    """
...
def allen_metadata(loom_file,
    ...
    neuron_valid = np.logical_and((df_col_metadata['Valid_QC'] == 1),
                                  (df_col_metadata['Category_Allen'] == 'Neuron'))
    non_valid = np.logical_and((df_col_metadata['Valid_QC'] == 1), 
                                (df_col_metadata['Category_Allen'] == 'Non-Neuronal'))
