...
def label_covered_features(loom_file,
    ...
    Args:
        ...
        min_val (int/float): Mininum count for a feature to be considered covered (>=)
    ...
    with loompy.connect(loom_file) as ds:
        ...
        for (_,selection,view) in ds.scan(items = row_idx,
                                          ...
                                          axis = 0):
            min_num = np.sum(view.layers[layer][:,col_idx] >= min_count, axis = 1)
            if fraction_covered is None:
...
def label_covered_cells(loom_file,
    ...
    Args:
        ...
        out_attr (str): Name of row attribute specifying valid features
        min_val (int/float): Mininum count for a feature to be considered covered (>=)
        fraction_covered (float): Mininum fraction of features with coverage (>=)
    ...
    with loompy.connect(loom_file) as ds:
        ...
        for (_,selection,view) in ds.scan(items = col_idx,
                                          ...
                                          batch_size = batch_size,
                                          axis = 1):
            min_num = np.sum(view.layers[layer][row_idx,:] >= min_count, axis = 0)
            if fraction_covered is None:
